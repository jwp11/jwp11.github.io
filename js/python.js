/* ============================================
   Python 教程页:目录渲染 / 进度追踪 / 滚动高亮
   依赖 main.js 暴露的 BlogMD.renderMarkdown
   ============================================ */
(function () {
  "use strict";

  const PROGRESS_KEY = "python-progress";
  const tocNav = document.getElementById("tocNav");
  const content = document.getElementById("pyContent");
  const bar = document.getElementById("progressBar");
  const progressText = document.getElementById("progressText");
  if (!tocNav || !content || typeof PY_CHAPTERS === "undefined") return;
  ...
  tocNav.innerHTML = PY_CHAPTERS.map((ch) => `
  ...
  content.innerHTML = PY_CHAPTERS.map((ch) => `
  ...
  function refreshProgress() {
    const total = PY_CHAPTERS.length;
    const learned = PY_CHAPTERS.filter((c) => progress[c.id]).length;
